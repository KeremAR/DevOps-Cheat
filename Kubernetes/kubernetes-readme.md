...
*   `-f <values-file.yaml>`: Override default values using a YAML file. Helm merges the values from the specified file over the chart's default `values.yaml`. If a key exists in both, the value from your file takes precedence. This is the primary way to apply environment-specific configurations (e.g., `values-staging.yaml`, `values-prod.yaml`).
*   `--create-namespace`: If the release namespace does not exist, it will be created.
...
*   `--install`: If a release with the specified name does not exist, this flag will run `helm install` instead.
...
# Apply to all resources generated by this kustomization
commonLabels:
  app.kubernetes.io/name: todo-app
commonAnnotations:
  on-call-team: "sre-team-alpha"
```

**3. Customizing with Overlays**

The `overlays/staging/kustomization.yaml` file points to the `base` and applies environment-specific customizations. It can add prefixes to names, override annotations and labels, apply patches, and even add new resources.
...
# Add new resources specific to this overlay
resources:
- namespace.yaml
...
# Override common annotations from the base for this environment
commonAnnotations:
  deployment.environment: staging

# Apply patches to modify resources from the base.
# This example uses an inline JSON Patch.
patches:
- patch: |-
    - op: replace
      path: /spec/replicas
...
# The labels transformer can add labels to resources and their selectors.
labels:
